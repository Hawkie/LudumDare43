...
export function DrawSpriteAngled(
  ctx: DrawContext,
  x: number,
  y: number,
  angle: number,
  sprite: ISprite,
  spriteImg: IImageElement,
): void {
  if (spriteImg.loaded) {
    let xOffset: number =
      x + (sprite.frames[sprite.index].width * sprite.scaleX) / 2;
    let yOffset: number =
      y + (sprite.frames[sprite.index].height * sprite.scaleY) / 2;
    ctx.save();
