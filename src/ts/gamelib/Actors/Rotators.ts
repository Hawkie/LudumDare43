...
export function RotateAngle<T extends IAngled>(
  angled: T,
  spin: number,
  timeModifier: number,
): T {
...
export function RotateShape<T extends IAngledShape>(
  timeModifier: number,
  hasShape: T,
...
export function RotatePoly<T extends IShape>(
  timeModifier: number,
  shape: T,
  angularVelocity: number,
): T {
  ...
  let newPoints: ICoordinate[] = Transforms.RotateCoordinates(
    shape.points,
    rotateAngle,
  );
  let newOffset: ICoordinate = Transforms.RotateCoordinates(
    [shape.offset],
    rotateAngle,
  ).pop();
