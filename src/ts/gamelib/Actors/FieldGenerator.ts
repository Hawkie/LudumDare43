...
export function FieldGenerate<P, F extends IField<P>>(
  timeModifier: number,
  field: F,
  on: boolean,
...
function GenerationCheck<TState extends IGenerationState>(
  timeModifier: number,
  inputState: TState,
  particlesPerSecond: number,
): TState {
...
// add
function CreateAndAddParticles<P, F extends IField<P>>(
  timeModifier: number,
  starField: F,
  toAdd: number,
  func: (now: number) => P,
