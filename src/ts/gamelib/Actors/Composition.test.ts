...
function bark<T extends number>(woofFactor: T, action: number): T {
  return (woofFactor + action) as T;
}

// one example using this
function meow(this: ICat<number>, action: number): number {
  return this.cuteFactor + action;
}
