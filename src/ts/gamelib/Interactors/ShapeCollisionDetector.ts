...
export function ShapeCollisionDetector(
  target: IShapedLocation,
  hitters: ICoordinate[],
): number {
  return hitters.findIndex((h) =>
    Transforms.hasPoint(target.shape.points, target.location, h),
  );
}
...
export function ShapesCollisionDetector(
  targets: IShapedLocation[],
  hitters: ICoordinate[],
): IDetected {
  let i2: number = -1;
