...
export function NextTotal(state: IState): number {
  return state.behaviours[state.activeState].next(
    state.states[state.activeState],
  );
}

export function DisplayTotal(ctx: DrawContext, state: IState): void {
  state.behaviours[state.activeState].display(
    ctx,
    state.states[state.activeState],
  );
}
...
export function InputTotal(state: IState, eState: IEventState): IState {
  ...
  let newState: any = state.behaviours[state.activeState].input(
    state.states[state.activeState],
    eState,
  );
...
export function UpdateTotal(state: IState, timeModifier: number): IState {
  let newStates: any[] = state.states.map((s) => s);
  let newState: any = state.behaviours[state.activeState].update(
    state.states[state.activeState],
    timeModifier,
  );
  newStates.splice(state.activeState, 1, newState);
...
export function SoundTotal(state: IState, timeModifier: number): IState {
  let newStates: any[] = state.states.map((s) => s);
  let newState: any = state.behaviours[state.activeState].sound(
    state.states[state.activeState],
    timeModifier,
  );
  newStates.splice(state.activeState, 1, newState);
