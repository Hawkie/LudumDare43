...
export function CreateTestStateMachine(): IStateProcessor<ITestState> {
  return {
    ...
    input: InputTest,
    update: EmptyUpdate,
    next: (state: ITestState) => {
      return undefined;
    },
  };
}

export function EmptyInput<T>(
  state: T,
  eState: IEventState,
  timeModifier: number,
): T {
...
export function InputTest(
  ...
  return { ...state, controls: Click(state.controls, eState) };
}
...
export function DisplayTest(ctx: DrawContext, state: ITestState): void {
  ...
  state.errors.forEach((e) => {
    DrawText(ctx, 100, y, e);
    y += 15;
  });

  // keys
  if (state.controls.keys !== undefined) {
    state.controls.keys.forEach((k) => {
      DrawNumber(ctx, 100, y, k);
      y += 15;
    });
  }
  ...
  // draw drag line
  if (
    state.controls.start !== undefined &&
    state.controls.current !== undefined &&
    state.controls.press
  ) {
    DrawLine(
      ctx,
      state.controls.start.x,
      state.controls.start.y,
      state.controls.current.x,
      state.controls.current.y,
    );
  }
  let rSize: number = 10;
  ...
  if (state.controls.touches !== undefined) {
    ...
    for (let i: number = 0; i < state.controls.touches.length; i++) {
      DrawCircle(
        ctx,
        state.controls.touches[i].clientX,
        state.controls.touches[i].clientY,
        30,
      );
    }
  }
