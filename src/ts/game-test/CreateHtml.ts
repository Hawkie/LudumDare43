...
export function CreateTestHtml(document: Document): Canvas {
  let b1: HTMLButtonElement = document
    .getElementById("div1")
    .appendChild(CreateButton(document, "Play mp3"));
  b1.onclick = (e) => {
    Game.assets.cinematic.play();
  };
  let b2: HTMLButtonElement = document
    .getElementById("div1")
    .appendChild(CreateButton(document, "Play wav"));
  b2.onclick = (e) => {
