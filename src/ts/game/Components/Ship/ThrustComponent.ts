...
export function ExhaustCopyToUpdated(
  timeModifier: number,
  exhaust: IExhaust,
  on: boolean,
  x: number,
  ...
  let velocity: ICoordinate = Transforms.VectorToCartesian(
    angle + Transforms.random(-5, 5),
    100 + Transforms.random(-5, 5),
  );
  return {
    ...exhaust,
    thrustOn: on,
    exhaustParticleField: FieldGenRemMove(
      timeModifier,
      exhaust.exhaustParticleField,
      on,
      50,
      0.5,
      (now: number) => {
