...
export function DisplayWeapon(ctx: DrawContext, weapon: IWeapon): void {
  weapon.bullets.forEach((p) =>
    DrawGraphic(ctx, p.x - 15, p.y - 15, Game.assets.fallingMan),
  );
}
...
export function PullTrigger(
  ...
  if (reloaded) {
    fired = true;
    let velocity: Coordinate = Transforms.VectorToCartesian(
      angle,
      bulletVelocity,
    );
  ...
  let bulletsFalling: IParticle[] = bullets
    .filter((b) => b.size !== 1)
    .map((b) => AccelerateWithForces(b, timeModifier, [gravity], 1));
  bulletsFalling = bulletsFalling.map((b) =>
    MoveWithVelocity(timeModifier, b, b.Vx, b.Vy),
  );
