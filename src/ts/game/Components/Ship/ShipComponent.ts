...
import {
  IExhaust,
  ExhaustCopyToUpdated,
  DisplayExhaust,
  CreateExhaust,
} from "./ThrustComponent";
import {
  IExplosion,
  DisplayExplosion,
  CreateExplosion,
  UpdateExplosion,
} from "./ExplosionComponent";
...
export function DisplayShip(ctx: DrawContext, ship: IShip): void {
  DrawPoly(
    ctx,
    ship.x + ship.shape.offset.x,
    ship.y + ship.shape.offset.y,
    ship.shape,
  );
...
export function ShipCopyToUpdated(
  timeModifier: number,
  ship: IShip,
  controls: IControls,
): IShip {
...
function ShipSubComponents(ship: IShip, timeModifier: number): IShip {
  let w: IWeapon = PullTrigger(
    ...
    ship.Vy,
    ship.angle,
    ship.weapon1.bulletVelocity,
  );
  return {
    ...ship,
    exhaust: ExhaustCopyToUpdated(
      timeModifier,
      ship.exhaust,
      ship.forwardThrust > 0,
      ship.x,
      ship.y,
      ship.Vx,
      ship.Vy,
...
export function CrashShip(ship: IShip, Vx: number, Vy: number): IShip {
  ...
  return { ...ship, crashed: true, Vx: Vx, Vy: Vy, guiDescent: guiDescent };
}

// change to green
export function LandShip(ship: IShip): IShip {
  return { ...ship, Vx: 0, Vy: 0, landed: true };
}

export function ShipCopyToRemovedBullet(
  ship: IShip,
  bulletIndex: number,
): IShip {
  return { ...ship, weapon1: RemoveBullet(ship.weapon1, bulletIndex) };
}

export function ShipCopyToStopBullet(ship: IShip, bulletIndex: number): IShip {
  return { ...ship, weapon1: StopBullet(ship.weapon1, bulletIndex) };
}
