...
export function SoundMenu(
  menuState: IMenuComponent,
  music: IAudioElement,
  changeSound: IAudioElement,
): IMenuComponent {
  music.play();
  if (menuState.moved) {
    changeSound.replay();
    // turn off moved once sound played (can optimise this in replay perhaps?)
    return { ...menuState, moved: false };
  }
  // unchanged state
  return menuState;
}
...
export function InputMenu(
  ...
  if (now - lastMoved > 150) {
    ...
    if (eState.current !== undefined) {
      let focus1: number = Math.max(
        0,
        Math.min(
          menu.menuItems.length - 1,
          Math.round((eState.current.y - 100) / 50),
        ),
      );
      if (focus1 !== focus) {
        moved = true;
  ...
  if (eState.keys.indexOf(Keys.Enter) > -1 || eState.click) {
    selected = true;
  }
