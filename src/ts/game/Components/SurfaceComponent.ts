...
export function generatePoint(
  ...
  const yNew: number =
    yBase + Transforms.random(generator.lower, generator.upper);
...
export function initSurface(
  ...
  for (let i: number = 1; i < endIndex; i++) {
    let point: ICoordinate = generatePoint(
      i * generator.resolution,
      points[i - 1].y,
      generator,
    );
    points.push(point);
...
export function addSurface(
  ...
  return { ...surface, points: newPoints, addedLeft: addedLeft };
}
...
export function TestFlat(
  ...
  for (let i: number = 0; i < range; i++) {
    ...
    if (Math.abs(surface.points[index].y - height) > 2) {
      console.log(
        "Surface Not Flat! Under ship:" +
          height +
          ". Point" +
          surface.points[index].y,
      );
      return false;
    }
  }
