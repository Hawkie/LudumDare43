...
export function UpdateMenuState(
  ...
  return {
    ...
    balloonX: Math.max(
      200,
      Math.min(350, state.balloonX + Transforms.random(-5, 5) * timeModifier),
    ),
    balloonY: Math.max(
      100,
      Math.min(300, state.balloonY + Transforms.random(-5, 5) * timeModifier),
    ),
  };
}
