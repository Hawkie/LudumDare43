...
export function CreateGameStateLandExplorer(
  score: number,
): ILandExplorerGameState {
  ...
  let ship: IShip = CreateShip(
    Game.assets.width / 2,
    Game.assets.height / 2,
    10,
    MoveShip,
  );
  ...
  let state: ILandExplorerState = CreateLandExplorer(
    ship,
    starfield,
    surface,
    score,
...
export function Update(
  ...
  return {
    ...
    view: Zoom(
      state.view,
      newState.controls.zoomIn,
      newState.controls.zoomOut,
      newState.ship.y,
    ),
  };
}
...
export function Sounds(state: ILandExplorerGameState): ILandExplorerGameState {
  ...
  return { ...state, landState: newLandState };
}

export function InputGameState(
  state: ILandExplorerGameState,
  eState: IEventState,
): ILandExplorerGameState {
  let newState: ILandExplorerState = InputState(state.landState, eState);
  return { ...state, landState: newState };
}
...
export function Display(ctx: DrawContext, state: ILandExplorerGameState): void {
  ...
  DisplayView(
    ctx,
    state.view,
    state.landState.ship.x,
    512 - (512 + 123 - state.landState.ship.y) / 2,
    state.landState,
    { displayState: DisplayLandExplorer },
  );
...
function DisplayGUI(ctx: DrawContext, state: ILandExplorerGameState): void {
  ...
  DrawGraphic(
    ctx,
    Game.assets.width - 50,
    Game.assets.height * 2 - 100,
    Game.assets.surprise,
  );
  ctx.zoom(2, 2);
  ...
  DrawNumber(
    ...
    Game.assets.width - x,
    y,
    state.landState.ship.x - Game.assets.width / 2,
  );
  // passengers
  y += 20;
  ...
  DrawNumber(
    ...
    Game.assets.width - x,
    y,
    state.landState.ship.weapon1.remaining,
  );
  // fuel
  y += 20;
  ...
  DrawNumber(
    ...
    Game.assets.width - x,
    y,
    Math.abs(Game.assets.height - state.landState.ship.y),
  );
