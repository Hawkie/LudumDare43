import {
  ...
  ShipSounds,
  LandShip,
  ShipCopyToStopBullet,
} from "../../Components/Ship/ShipComponent";
import {
  ISurface,
  DisplaySurface,
  addSurface,
  TestFlat,
} from "../../Components/SurfaceComponent";
import { IParticleField } from "../../Components/FieldComponent";
import {
  IControls,
  InputControls,
  CreateControls,
} from "../../Components/ControlsComponent";
import {
  IEventState,
  CreateEventState,
  Click,
} from "../../../gamelib/Events/EventProcessor";
import { DrawContext } from "../../../gamelib/1Common/DrawContext";
import {
  DisplayField,
  FieldGenMove,
  IParticle,
} from "../../../gamelib/Components/ParticleFieldComponent";
import { Transforms } from "../../../gamelib/Physics/Transforms";
import { Game } from "../../../gamelib/1Common/Game";
import {
  IShapedLocation,
  ShapeCollisionDetector,
} from "../../../gamelib/Interactors/ShapeCollisionDetector";
import {
  ISplat,
  CreateSplat,
  DisplaySplat,
  UpdateSplat,
  ResetSplat,
} from "../../Components/SplatComponent";
...
export function CreateLandExplorer(
  ship: IShip,
  starfield: IParticleField,
  surface: ISurface,
  residualScore: number = 0,
): ILandExplorerState {
  return {
    title: "Air Rider",
...
export function DisplayLandExplorer(
  ctx: DrawContext,
...
export function LandExplorerSounds(
  state: ILandExplorerState,
): ILandExplorerState {
...
export function StateCopyToUpdate(
  state: ILandExplorerState,
  timeModifier: number,
): ILandExplorerState {
  ...
  let score: number =
    state.residualScore +
    (((state.ship.x - Game.assets.width / 2) / 50) *
      state.ship.weapon1.remaining) /
      8;
  ...
  return {
    ...
    surface: addSurface(
      ...
      state.ship.y,
      Game.assets.width,
      state.surface.surfaceGenerator,
    ),
    score: score,
    splat: splat,
  };
...
function TouchLand(state: ILandExplorerState): ILandExplorerState {
  if (
    ...
    if (!state.ship.crashed && TestLand(state)) {
      return { ...state, ship: LandShip(state.ship) };
    } else {
      return { ...state, ship: CrashShip(state.ship, 0, 0), score: 0 };
    }
  }
  return state;
}
...
function ManHitsLand(state: ILandExplorerState): ILandExplorerState {
  ...
  const indexPassenger: number = ShapeCollisionDetector(
    surfaceShape,
    state.ship.weapon1.bullets.slice(state.splatIndex + 1).map((b) => {
      return {
        x: b.x,
        y: b.y,
      };
